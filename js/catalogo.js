...
const cargarCatalogo = (callback) => {
    fetch('productos.json')
        ...
        .then(data => {
            catalogo = data.map(item => new Producto(item));
            callback();
        })
        .catch(error => {
            alert(`Error al cargar productos. Error: ${error.message}`);
        });
};
...
const renderProducts = (lista) => {
    ...
    lista.forEach((product, index) => {
        ...
        productCard.innerHTML =
            ...
            <input type="number" id="qty-${index}" class="form-control mb-2" placeholder="Cantidad" min="1" max="${product.stock}"${agotado ? ' disabled' : ''}>
            <input type="checkbox" class="btn-check" id="chk-${index}" autocomplete="off"${agotado ? ' disabled' : ''}>
            <label class="btn btn-outline-primary mb-4" for="chk-${index}">${agotado ? 'Sin stock' : 'Agregar al carrito'}</label>
            <button class="btn btn-primary w-100 bg-1" onclick="addToCart(${index})"${agotado ? ' disabled' : ''}>Agregar</button>
            </div>
            </div>`;
...
const renderCart = () => {
    ...
    cart.forEach((product, index) => {
        tableHtml +=
            `<tr>
            <td>${product.title}</td>
            <td><input type="number" min="0" max="${product.stock}" value="${product.quantity}" onchange="updateCartQuantity(${index}, this.value)" class="form-control" style="width:80px"></td>
            <td>${product.price}</td>
...
const calculateTotals = () => {
    const neto = cart.reduce((total, item) => total + item.getTotal(), 0);
    const iva = neto * 0.19;
    let total = neto + iva;
...
if (formCompra) {
    formCompra.addEventListener('submit', (e) => {
        ...
        cart.forEach(item => {
            const producto = catalogo.find(prod => prod.code === item.code);
            if (producto) {
